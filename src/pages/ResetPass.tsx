...
export default function ResetPass() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      const { data } = await postWithToken('/resetpass', { password }, token)
    ...
    <div style={{ maxWidth: 520, margin: '30px auto' }}>
      <div className="card">
        ...
        <form onSubmit={onSubmit} className="grid" style={{gap: 12}}>
          ...
          <button className="btn" type="submit" disabled={pending}>{pending ? 'Saving…' : 'Update Password'}</button>
        </form>
        <p className="muted" style={{marginTop:10}}>This sends <code>Authorization: Bearer &lt;resetToken&gt;</code> to <code>/resetpass</code>.</p>
      </div>
    </div>
  )
}
